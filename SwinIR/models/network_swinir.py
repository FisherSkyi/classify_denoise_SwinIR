...
try:
    ...
    # ---- Minimal fall-back implementations (no external deps) --------
    import math
    import torch
    from torch import nn

    # Converts an int to a size-2 tuple; leaves tuples unchanged
    def to_2tuple(x):
        return (x, x) if not isinstance(x, tuple) else x

    class DropPath(nn.Module):
        """
        DropPath (a.k.a. stochastic depth) per sample.
        Works for arbitrary tensor shapes; the first dimension is treated
        as the batch dimension and kept / dropped uniformly.
        """
        ...
        def forward(self, x):
            ...
            # Generate binary mask – shape: (B, 1, 1, ..., 1)
    ...
    def trunc_normal_(tensor, mean: float = 0.0, std: float = 1.0,
        ...
        """
        In-place truncated normal initialisation (±2σ by default).
        Uses inverse-error-function transformation so that the results
        match those produced by timm / TensorFlow / JAX.
        """
        with torch.no_grad():
            ...
            # Compute upper & lower cdf values
            l, u = norm_cdf((a - mean) / std), norm_cdf((b - mean) / std)
            tensor.uniform_(l, u)           # Uniform in [l, u]
            tensor.erfinv_()                # Transform to normal
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=3,
                 embed_dim=32, depths=[6, 6, 6, 6], num_heads=[6, 6, 6, 6],
